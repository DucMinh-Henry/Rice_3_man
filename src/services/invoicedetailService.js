...
const getInvoiceDetails = async () => {
  let [results, fields] = await connection.query(
    "SELECT * FROM `rice_4_man`.`InvoiceDetails`;"
  );
  return results;
};
...
const getInvoiceDetailid = async (id) => {
  let [results, fields] = await connection.query(
    "SELECT * FROM `rice_4_man`.`InvoiceDetails` WHERE idInvoiceDetails = ?;",
    [id]
  );
  return results;
};

const queryupdateInvoiceDetail = async (
  idInvoiceDetails,
  idInvoice,
  idProduct,
  nameProduct,
  ...
  let query =
    "UPDATE `rice_4_man`.`InvoiceDetails` SET `idInvoice` = ?, `idProduct` = ?, `nameProduct` = ?, `quanity` = ?, `price` = ?, `urlImage` = ?, `Invoice_idInvoice` = ?, `Product_idProduct` = ? WHERE `idInvoiceDetails` = ?";
  let params = [
    idInvoice,
    idProduct,
    nameProduct,
...
const getidmax = async () => {
  let [results, fields] = await connection.query(
    "SELECT MAX(`idInvoiceDetails`) as max FROM `rice_4_man`.`InvoiceDetails`"
  );
